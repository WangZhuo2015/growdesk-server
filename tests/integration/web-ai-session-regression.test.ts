...
function ownedRun(): Run {
  ...
  const file = fs.realpathSync(manifest);
...
test("durable Web AI: Fastify injection with owned PostgreSQL", async t => {
  const run = ownedRun();
  ...
  const jwtSecret = "test_web_ai_state_secret_at_least_32_characters";
  const app = buildApiApp({ databaseContext: database, jwtSecret });
  const users: string[] = [];
  ...
  await t.test("create preserves context and scoped metadata", async () => {
    ...
    assert.equal(session.contextType, "food");
  ...
  await t.test("fresh service and application instances read persisted history", async () => {
    const session = await new WebAiSessionService(database.pool).get(a.userId, sessionId);
    assert.deepEqual(session.messages.map(m => m.content), ["test_question", "test_answer"]);
    assert.equal(session.messageCount, 2);
    const secondApp = buildApiApp({ databaseContext: database, jwtSecret });
    try {
      const response = await secondApp.inject({ method: "GET", url: `${endpoint}/${sessionId}`, headers: a.headers });
      assert.equal(response.statusCode, 200, response.payload);
      assert.equal(response.json<{ data: WebAiSession }>().data.messageCount, 2);
  ...
  await t.test("rename persists and revoked baby membership blocks existing credentials", async () => {
    ...
    await database.pool.query("UPDATE baby_members SET status='revoked' WHERE user_id=$1 AND baby_id=$2", [a.userId, a.babyId]);
    try {
      ...
      const hidden = await app.inject({ method: "GET", url: endpoint, headers: a.headers });
      assert.equal(hidden.json<{ data: { total: number } }>().data.total, 0);
    } finally { await database.pool.query("UPDATE baby_members SET status='active' WHERE user_id=$1 AND baby_id=$2", [a.userId, a.babyId]); }
  });
  await t.test("concurrent identical submissions persist only one message", async () => {
    const payload = { id: randomUUID(), role: "user", content: "test_concurrent_message" };
    const responses = await Promise.all(Array.from({ length: 4 }, () => app.inject({ method: "POST", url: `${endpoint}/${sessionId}/messages`, headers: a.headers, payload })));
    for (const response of responses) assert.equal(response.statusCode, 201, response.payload);
  ...
  await t.test("invalid input does not modify stored history", async () => {
    ...
    assert.equal((await new WebAiSessionService(database.pool).get(a.userId, sessionId)).messageCount, before.messageCount);
  });
  await t.test("explicit deletion removes stored history without local resurrection", async () => {
