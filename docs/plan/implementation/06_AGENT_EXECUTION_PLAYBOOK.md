...
状态：尚无任务的独立验收记录。用户已建立部分 BOOT 骨架，开工先核对已有产物，再记录 NOT_STARTED / IMPLEMENTED_NOT_REVIEWED；不可据此直接标记 ACCEPTED。本文不是让Agent一次完成整个项目的巨大提示词。一次只领取一个任务；复杂任务按本文子步骤拆成小提交，完成证据后才领取依赖任务。任务负责人不能自行把功能删出首发范围。
