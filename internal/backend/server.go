...
func NewServer(ctx context.Context, c Config, log *slog.Logger) (*Server, error) {
	...
	if err != nil { pool.Close(); return nil, errors.New("invalid Redis configuration") }
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Body != nil && r.ContentLength != 0 {
		...
		if err != nil { s.writeError(w, invalid("Cannot read request body"), requestID); return }
	...
	if !s.Public[route.OperationID] {
		...
		if err != nil { s.writeError(w, err, requestID); return }
		req.Principal = p
	}
	h := s.Handlers[route.OperationID]
	if h == nil { s.writeError(w, apiError(503, "GO_OPERATION_NOT_IMPLEMENTED", "Operation is not implemented in this Go build"), requestID); return }
	result, err := h(ctx, req)
	if err != nil { s.writeError(w, err, requestID); return }
	if result.Status == 0 { result.Status = 200 }
	for k, values := range result.Headers {
		for _, v := range values { w.Header().Add(k, v) }
	}
	if result.Stream != nil {
		if err := result.Stream(w); err != nil { s.Log.Warn("stream interrupted", "operationId", route.OperationID, "requestId", requestID) }
		return
	}
	...
	if r.Method != http.MethodHead { _, _ = w.Write(raw) }
}
func (s *Server) writeError(w http.ResponseWriter, err error, id string) {
	...
	if e.Details != nil { body["details"] = e.Details }
