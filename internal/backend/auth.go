...
func (s *Server) authenticate(ctx context.Context, r *http.Request) (Principal, error) {
	...
	if err != nil || !token.Valid || text(claims["typ"]) != "at+jwt" || text(claims["sub"]) == "" || text(claims["sid"]) == "" {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return Principal{}, apiError(401, "SESSION_REVOKED", "Session has expired or was revoked")
	}
	if err != nil {
...
func (s *Server) passwordHash(ctx context.Context, password string, cost int) (string, error) {
	...
	b, err := bcrypt.GenerateFromPassword(bcryptPassword(password), cost)
	if err != nil {
		return "", err
	}
	// Go emits the equivalent modern bcrypt algorithm with a 2a marker.
	// Emit 2b like bcryptjs, otherwise every native login requests a rehash.
	return "$2b$" + string(b[4:]), nil
}
...
func userDTO(row Object) Object {
	return Object{"id": row["id"], "username": row["username"], "displayName": row["display_name"], "createdAt": isoValue(row["created_at"]), "updatedAt": isoValue(row["updated_at"])}
}
func inferPlatform(label string) string {
	label = strings.ToLower(label)
	for _, entry := range []struct {
		kind  string
		words []string
	}{{"ios", []string{"ios", "iphone", "ipad"}}, {"web", []string{"web", "chrome", "safari", "firefox"}}, {"macos", []string{"mac"}}, {"android", []string{"android"}}} {
...
type sessionTokens struct {
	SessionID, RefreshToken, RotationID, AccessToken string
	ExpiresAt                                        time.Time
}
...
func (s *Server) createSession(ctx context.Context, q Querier, user, label, platform string) (sessionTokens, error) {
	...
	_, err := q.Exec(ctx, `INSERT INTO device_sessions(id,user_id,device_label,platform,absolute_expires_at) VALUES($1,$2,$3,$4,$5)`, x.SessionID, user, label, platform, time.Now().UTC().Add(30*24*time.Hour))
	if err != nil {
		return x, err
	}
	_, err = q.Exec(ctx, `INSERT INTO refresh_credentials(token_hash,session_id,user_id,rotation_id,expires_at) VALUES($1,$2,$3,$4,$5)`, hashText(x.RefreshToken), x.SessionID, user, x.RotationID, x.ExpiresAt)
	if err != nil {
		return x, err
	}
	x.AccessToken, err = s.signAccess(user, x.SessionID, label)
	return x, err
}

func (s *Server) registerAuth() {
	s.Register("register", true, s.registerAccount)
	s.Register("login", true, s.login)
	s.Register("refreshSession", true, s.refresh)
	s.Register("exchangeBffSession", true, s.bffExchange)
	s.Register("revokeBffSession", true, s.bffRevoke)
	s.Register("logout", false, s.logout)
...
func (s *Server) registerAccount(ctx context.Context, r *Request) (Result, error) {
	...
	var collision bool
	if err := s.DB.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM users WHERE username=$1)", name).Scan(&collision); err != nil {
		return Result{}, err
	}
	if collision {
		return Result{}, apiError(409, "USERNAME_EXISTS", "Username '"+name+"' is already taken")
	}
	hash, err := s.passwordHash(ctx, password, 10)
	if err != nil {
		return Result{}, err
	}
	...
	user, err := one(ctx, tx, `INSERT INTO users AS u(id,username,password_hash,display_name,updated_at) VALUES($1,$2,$3,$4,NOW()) RETURNING to_jsonb(u)`, userID, name, hash, display)
	if err != nil {
		if normalizedError(err).Status == 409 {
			return Result{}, apiError(409, "USERNAME_EXISTS", "Username '"+name+"' is already taken")
		}
		return Result{}, err
	}
	...
	tokens, err := s.createSession(ctx, tx, userID, text(r.Body["deviceLabel"]), "")
	if err != nil {
		return Result{}, err
	}
...
func (s *Server) verifyCredentials(ctx context.Context, username, password string) (Object, error) {
	...
	if !valid || user == nil || user["deleted_at"] != nil {
		return nil, apiError(401, "INVALID_CREDENTIALS", "Invalid username or password")
	}
	return user, nil
}
func (s *Server) me(ctx context.Context, r *Request) (Result, error) {
...
func (s *Server) listSessions(ctx context.Context, r *Request) (Result, error) {
	...
	for _, row := range rows {
		items = append(items, Object{"id": row["id"], "deviceLabel": row["device_label"], "createdAt": isoValue(row["created_at"]), "lastSeenAt": isoValue(row["last_seen_at"]), "expiresAt": isoValue(row["absolute_expires_at"])})
	}
	return ok(items)
}
...
func (s *Server) revoke(ctx context.Context, user, session string) (bool, error) {
	...
	defer rollback(tx)
	var cursor int64
	if err = tx.QueryRow(ctx, "SELECT cursor FROM user_sync_states WHERE user_id=$1 FOR UPDATE", user).Scan(&cursor); err != nil {
		return false, err
	}
	done, err := revokeSessionTx(ctx, tx, user, session)
	if err != nil {
		return false, err
	}
	return done, tx.Commit(ctx)
}
func (s *Server) logout(ctx context.Context, r *Request) (Result, error) {
	_, err := s.revoke(ctx, r.Principal.UserID, r.Principal.SessionID)
	if err != nil {
		return Result{}, err
	}
	return ok(Object{"success": true})
}
func (s *Server) revokeSessionHandler(ctx context.Context, r *Request) (Result, error) {
	done, err := s.revoke(ctx, r.Principal.UserID, r.Params["id"])
	if err != nil {
		return Result{}, err
	}
	if !done {
		return Result{}, apiError(404, "SESSION_NOT_FOUND", "Session '"+r.Params["id"]+"' was not found or belongs to another user")
	}
	return ok(Object{"revoked": true})
}
...
func (s *Server) changePassword(ctx context.Context, r *Request) (Result, error) {
	...
	user, err := one(ctx, tx, "SELECT to_jsonb(u) FROM users u WHERE id=$1 AND deleted_at IS NULL", r.Principal.UserID)
	if err != nil {
		return Result{}, err
	}
	valid, err := s.checkPassword(ctx, text(r.Body["oldPassword"]), text(user["password_hash"]))
	if err != nil {
		return Result{}, err
	}
	if !valid {
		return Result{}, apiError(401, "INVALID_CREDENTIALS", "Current password does not match")
	}
	hash, err := s.passwordHash(ctx, text(r.Body["newPassword"]), 12)
	if err != nil {
		return Result{}, err
	}
...
func normalizeRecovery(raw string) string {
...
func (s *Server) regenerateCodes(ctx context.Context, r *Request) (Result, error) {
	...
	valid, err := s.checkPassword(ctx, text(r.Body["password"]), text(user["password_hash"]))
	if err != nil {
		return Result{}, err
	}
	if !valid {
	...
	if _, err = tx.Exec(ctx, "UPDATE recovery_codes SET revoked_at=NOW() WHERE user_id=$1 AND revoked_at IS NULL AND used_at IS NULL", r.Principal.UserID); err != nil {
		return Result{}, err
	}
	batch := newID()
	...
	for i := 0; i < 10; i++ {
		...
		for j := 0; j < 32; j += 4 {
			chunks = append(chunks, raw[j:j+4])
		}
		codes = append(codes, strings.Join(chunks, "-"))
		if _, err = tx.Exec(ctx, "INSERT INTO recovery_codes(code_hash,user_id,batch_id) VALUES($1,$2,$3)", hashText(raw), r.Principal.UserID, batch); err != nil {
			return Result{}, err
		}
	}
	if _, err = tx.Exec(ctx, "UPDATE user_sync_states SET cursor=cursor+1,updated_at=NOW() WHERE user_id=$1", r.Principal.UserID); err != nil {
		return Result{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return Result{}, err
	}
	return ok(Object{"codes": codes, "batchId": batch})
}
func (s *Server) recoverPassword(ctx context.Context, r *Request) (Result, error) {
	...
	code := normalizeRecovery(text(r.Body["recoveryCode"]))
	if len(code) != 32 {
		return Result{}, deny
	}
	tx, err := s.DB.Begin(ctx)
	...
	defer rollback(tx)
	user, err := one(ctx, tx, "SELECT to_jsonb(u) FROM users u WHERE username=$1 AND deleted_at IS NULL", text(r.Body["username"]))
	if errors.Is(err, pgx.ErrNoRows) {
		return Result{}, deny
	}
	if err != nil {
	...
	if rec["used_at"] != nil || rec["revoked_at"] != nil {
		return Result{}, deny
	}
	hash, err := s.passwordHash(ctx, text(r.Body["newPassword"]), 12)
	if err != nil {
