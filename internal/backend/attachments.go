...
func authorizeAttachment(ctx context.Context, q Querier, userID string, row Object, write, uploaderOnly bool) error {
	...
	if write && role == "viewer" {
		return apiError(403, "FAMILY_ACCESS_DENIED", "Family write permission is required")
	}
	if babyID := text(row["baby_id"]); babyID != "" {
		...
		if scope.FamilyID != familyID {
			return apiError(403, "BABY_SCOPE_MISMATCH", "Attachment scope is inconsistent")
		}
	}
...
func attachmentRow(ctx context.Context, q Querier, id string, lock bool) (Object, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, notFound("Attachment", id)
	}
	return row, err
}
...
func (s *Server) readNativeAttachment(ctx context.Context, userID, id string, write, uploaderOnly bool) (Object, error) {
	...
	_, err := s.readSnapshot(ctx, func(q Querier) (Result, error) {
		...
		if err = authorizeAttachment(ctx, q, userID, row, write, uploaderOnly); err != nil {
			return Result{}, err
		}
		result = row
		return Result{}, nil
...
func (s *Server) attachmentMutation(ctx context.Context, r *Request, uploaderOnly bool, apply func(pgx.Tx, Object) (Result, error)) (Result, error) {
	...
	defer rollback(tx)
	if _, err = lockFamily(ctx, tx, text(previous["family_id"])); err != nil {
		return Result{}, err
	}
	...
	if text(row["family_id"]) != text(previous["family_id"]) {
		return Result{}, apiError(409, "CONCURRENCY_CONFLICT", "Attachment scope changed")
	}
	if err = authorizeAttachment(ctx, tx, r.Principal.UserID, row, true, uploaderOnly); err != nil {
		return Result{}, err
	}
...
func attachmentDigest(raw string) (string, error) {
	...
	if err != nil || len(decoded) != 32 {
		return "", invalid("sha256 must be a 64-character hexadecimal digest")
	}
...
func (s *Server) createNativeAttachment(ctx context.Context, r *Request) (Result, error) {
	...
	if !attachmentMIMEs[mime] || size < 1 || size > maximum {
		return Result{}, invalid("Unsupported attachment type or size")
	}
	...
	if babyID != "" {
		...
		if familyID != "" && familyID != scope.FamilyID {
			return Result{}, apiError(403, "BABY_SCOPE_MISMATCH", "Attachment family does not own the baby")
		}
	...
	if err = authorizeAttachment(ctx, tx, r.Principal.UserID, metadata, true, false); err != nil {
		return Result{}, err
	}
	...
	if err = journalObjectPurge(ctx, tx, familyID, id, key, expires.Add(time.Hour)); err != nil {
		return Result{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return Result{}, err
...
func (s *Server) markAttachmentVerificationFailed(ctx context.Context, r *Request) error {
	_, err := s.attachmentMutation(ctx, r, true, func(tx pgx.Tx, row Object) (Result, error) {
		if text(row["status"]) != "ready" {
			if _, err := tx.Exec(ctx, "UPDATE attachments SET status='failed',updated_at=NOW() WHERE id=$1", row["id"]); err != nil {
				return Result{}, err
			}
		}
...
func (s *Server) completeNativeAttachment(ctx context.Context, r *Request) (Result, error) {
	...
	if digest != strings.TrimSpace(text(previous["sha256"])) || integer(r.Body["byteSize"]) != integer(previous["byte_size"]) {
		if err := s.markAttachmentVerificationFailed(ctx, r); err != nil {
			return Result{}, err
		}
	...
	if err != nil {
		if normalizedError(err).Status == 400 {
			if updateErr := s.markAttachmentVerificationFailed(ctx, r); updateErr != nil {
	...
	// Reserve cleanup durably before external I/O. If sealing or final SQL
	// fails, the scheduler removes this unreferenced key after the grace period.
	sealed := "families/" + text(previous["family_id"]) + "/attachments/sealed/" + text(previous["id"]) + "/" + newID()
	if err = journalObjectPurge(ctx, s.DB, text(previous["family_id"]), text(previous["id"]), sealed, time.Now().Add(time.Hour)); err != nil {
		return Result{}, err
	}
	if err = store.seal(ctx, sealed, text(previous["mime_type"]), integer(previous["byte_size"]), digest, file); err != nil {
		return Result{}, err
	}
...
func (s *Server) renewNativeAttachment(ctx context.Context, r *Request) (Result, error) {
	...
	return s.attachmentMutation(ctx, r, true, func(tx pgx.Tx, row Object) (Result, error) {
		if text(row["status"]) != "pending" {
			return Result{}, invalid("Only pending uploads can be renewed")
		}
		...
		if _, err = tx.Exec(ctx, "UPDATE attachments SET expires_at=$2,updated_at=NOW() WHERE id=$1", row["id"], expires); err != nil {
			return Result{}, err
		}
		if err = journalObjectPurge(ctx, tx, text(row["family_id"]), text(row["id"]), text(row["object_key"]), expires.Add(time.Hour)); err != nil {
			return Result{}, err
		}
...
func (s *Server) getNativeAttachmentContent(ctx context.Context, r *Request) (Result, error) {
	...
	if text(row["status"]) != "ready" {
		return Result{}, invalid("Attachment is not ready")
	}
	store, err := requireObjectStore(s)
	if err != nil {
		return Result{}, err
	...
	return Result{Status: 200, Stream: func(w http.ResponseWriter) error {
		release, err := acquireAttachmentIO()
		if err != nil {
			s.writeError(w, err, r.RequestID)
			return nil
		}
		defer release()
...
func (s *Server) deleteNativeAttachment(ctx context.Context, r *Request) (Result, error) {
	...
	return s.attachmentMutation(ctx, r, false, func(tx pgx.Tx, row Object) (Result, error) {
		...
		err := tx.QueryRow(ctx, `SELECT
			...
			EXISTS(SELECT 1 FROM ai_archive_entries WHERE attachment_id=$1) OR
            EXISTS(SELECT 1 FROM task_outbox o JOIN task_executions t ON t.id=o.aggregate_id
              WHERE t.status IN ('queued','running','cancelling','awaiting_confirmation')
                AND (o.payload->'__native'->'attachmentIds') @> jsonb_build_array($1::text))`, id, "/api/attachments/"+id).Scan(&referenced)
		if err != nil {
			return Result{}, err
		}
		if referenced {
			return Result{}, apiError(409, "ATTACHMENT_IN_USE", "Attachment is still referenced by family data")
		}
		purgeAt := time.Now()
		// A still-valid staging PUT capability can recreate an object after
		// deletion. Keep the cleanup due after its last issuance plus grace.
		if !strings.Contains(text(row["object_key"]), "/attachments/sealed/") {
			expires, e := asTime(row["expires_at"])
			if e != nil {
				return Result{}, e
			}
			if expires.Add(time.Hour).After(purgeAt) {
				purgeAt = expires.Add(time.Hour)
			}
		}
		if err = journalObjectPurge(ctx, tx, text(row["family_id"]), id, text(row["object_key"]), purgeAt); err != nil {
			return Result{}, err
		}
		if _, err = tx.Exec(ctx, "UPDATE attachments SET deleted_at=NOW(),status='failed',updated_at=NOW() WHERE id=$1", id); err != nil {
			return Result{}, err
		}
		return ok(Object{"success": true})
	})
