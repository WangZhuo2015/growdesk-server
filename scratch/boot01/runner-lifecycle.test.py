...
spec = importlib.util.spec_from_file_location('runner', Path(__file__).with_name('run-isolated.py'))
runner = importlib.util.module_from_spec(spec)
spec.loader.exec_module(runner)

class LifecycleTests(unittest.TestCase):
    def test_occupied_port_is_not_adopted(self):
        before = set(Path(tempfile.gettempdir()).glob('growdesk-boot01-*'))
        ...
        self.assertEqual(set(Path(tempfile.gettempdir()).glob('growdesk-boot01-*')), before)
    ...
    def test_wrong_instance_token_rejected_before_ddl(self):
        ...
        def wrong_token(args, **kwargs):
            if args == ['node', '--import', 'tsx', 'prisma-pg-check.ts']:
                manifest = Path(kwargs['env']['BOOT01_ENV_FILE'])
                ...
                result = subprocess.check_output([psql, '-h', identity['directory'], '-p', str(identity['pgPort']),
                    '-d', identity['database'], '-At', '-c', "SELECT to_regclass('public.test_boot01_records') IS NULL"],
