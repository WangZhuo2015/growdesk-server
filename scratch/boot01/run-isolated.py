"""Run BOOT-01 against child processes owned exclusively by this invocation."""
import json
import os
...
def main():
    ...
    env = {k:v for k,v in os.environ.items() if not k.startswith('PG') and k not in
           ['DATABASE_URL', 'TEST_DATABASE_URL', 'BOOT01_ENV_FILE', 'REDIS_URL']}
    children = []
    with tempfile.TemporaryDirectory(prefix='growdesk-boot01-') as temp:
        ...
        identity = dict(directory=str(root), pgPort=pgport, redisPort=redisport,
                        token=secrets.token_hex(16), database='test_growdesk_boot01',
                        user='test_boot01', password=secrets.token_hex(24))
        manifest = root / 'environment.json'
        manifest.write_text(json.dumps(identity)); manifest.chmod(0o600)
        env['BOOT01_ENV_FILE'] = str(manifest)
        logs = []
        try:
            ...
            command([bins['psql'], '-h', str(root), '-p', str(pgport), '-d', 'postgres', '-v', 'ON_ERROR_STOP=1'],
                input=f"CREATE ROLE test_boot01 LOGIN NOSUPERUSER PASSWORD '{identity['password']}';",
            ...
            command(['node', 'node_modules/prisma/build/index.js', 'generate', '--schema', 'prisma/schema.prisma'], env=env)
            command(['node', '--import', 'tsx', 'prisma-pg-check.ts'], env=env)
            command(['node', '--import', 'tsx', 'bullmq-redis-check.ts'], env=env)
            print('Owned PostgreSQL/Redis integration checks passed; child processes exited naturally.', flush=True)
        finally:
