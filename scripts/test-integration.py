...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='growdesk-integration-') as temp:
        ...
        try:
            ...
            command(['node', '--import', 'tsx', '--test', 'tests/integration/infrastructure.test.ts', 'tests/integration/foundation-migration.test.ts', 'tests/integration/unit-of-work.test.ts', 'tests/integration/auth.test.ts'], env=env)
